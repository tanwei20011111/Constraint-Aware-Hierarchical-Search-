...
def main() -> None:
    ...
    parser.add_argument("--input-path", help="要分析的文本文件或目录；不要求 manifest.json")
    parser.add_argument("--dataset-path", help="原始数据集文件路径；用于推断规则名称")
    parser.add_argument("--dataset-name", help="数据集名称，同时作为规则名和输出目录名 (rules/<名称>/)；必须与 build --dataset-name 一致；默认从 dataset-path 或 manifest 目录推断")
    parser.add_argument("--manifest-path", help="可选：manifest.json 路径；提供时按 manifest 读取 chunk")
    parser.add_argument("--base-rule-file", help="可选：指定参考 rule_profiles.json")
    parser.add_argument("--base-profile", help="可选：指定参考 profile 名称")
    parser.add_argument("--sample-count", type=int, default=12, help="抽样 chunk 数量")
    parser.add_argument("--max-chunk-chars", type=int, default=2200, help="每个样本最多给模型的字符数")
    parser.add_argument("--output-root", default=str(ROOT / "rules"), help="规则输出根目录")
    parser.add_argument("--dry-run", action="store_true", help="只调用模型并打印结果，不写文件")
    parser.add_argument("--force", action="store_true", help="允许覆盖已有规则文件")
    parser.add_argument("--print-prompt", action="store_true", help="把发送给模型的 prompt 打印到 stderr")
